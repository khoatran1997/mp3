...
public:
    ...
    uint16_t read_reg(uint8_t address){
        ...
        while(!dreq->ReadBool());
        mp3cs->SetLow();
        spi.Transfer(0x3);//read op code
        spi.Transfer(address);
        d[0] = spi.Transfer(0x11);//MSB
        while(!dreq->ReadBool());
        d[1] = spi.Transfer(0x11);//LSB
        while(!dreq->ReadBool());
        mp3cs->SetHigh();
    ...
    void write_reg(uint8_t address, uint16_t data){
        ...
        while(!dreq->ReadBool());
        mp3cs->SetLow();
        spi.Transfer(0x02);//write op code
        spi.Transfer(address);
